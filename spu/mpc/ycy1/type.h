...
class AYcyTy : public TypeImpl<AYcyTy, RingTy, Secret, AShare> {
  using Base = TypeImpl<AYcyTy, RingTy, Secret, AShare>;

 public:
  using Base::Base;
  static std::string_view getStaticId() { return "ycy1.AShr"; }

  explicit AYcyTy(FieldType field, int owner_rank = -1) {
    field_ = field;
    owner_ = owner_rank;
  }

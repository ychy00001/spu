// Copyright 2021 Ant Group Co., Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//   http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include "spu/mpc/ycy1/io.h"

#include "iostream"
#include "yacl/crypto/tools/prg.h"
#include "yacl/crypto/utils/rand.h"

#include "spu/mpc/common/pub2k.h"
#include "spu/mpc/util/ring_ops.h"
#include "spu/mpc/ycy1/type.h"
#include "spu/mpc/ycy1/value.h"

namespace spu::mpc::ycy1 {

std::vector<ArrayRef> Ycy1Io::toShares(const ArrayRef& raw, Visibility vis,
                                       int owner_rank) const {
  YACL_ENFORCE(raw.eltype().isa<RingTy>(), "expected RingTy, got {}",
               raw.eltype());
  const auto field = raw.eltype().as<Ring2k>()->field();
  YACL_ENFORCE(field == field_, "expect raw value encoded in field={}, got={}",
               field_, field);

  if (vis == VIS_PUBLIC) {
    const auto share = raw.as(makeType<Pub2kTy>(field));
    return std::vector<ArrayRef>(world_size_, share);
  } else if (vis == VIS_SECRET) {
    // by default, make as arithmetic share.
    std::vector<ArrayRef> splits(world_size_);
    // start mock
    // 模拟分隔数据，让每方都拿到当前值-1，等获取值时，直接用减去的数进行计算返回，
    // 示例：6+5 最终计算: (5+5+5) + (4+4+4) = 27
    for (std::size_t i = 0; i < world_size_; i++) {
      ArrayRef item = DISPATCH_ALL_FIELDS(field, "_", [&]() {
        using OutT = ring2k_t;
        ArrayRef out = raw.clone();
        auto _out = ArrayView<OutT>(out);
        pforeach(0, out.numel(),
                 [&](int64_t idx) { _out[idx] = _out[idx] + 10; });
        return out;
      });
      splits[i] = item;
    }
    // end mock
    YACL_ENFORCE(splits.size() == 3, "expect 3PC, got={}", splits.size());
    std::vector<ArrayRef> shares;
    for (std::size_t i = 0; i < 3; i++) {
      shares.push_back(ycy1::makeAYcyTyShare(splits[i], field, owner_rank));
    }
    return shares;
  }

  YACL_THROW("unsupported vis type {}", vis);
}

/**
 *  在 bv.pt_type == PT_BOOL
 *    && vtype == VIS_SECRET
 *    && base_io_->hasBitSecretSupport()
 *  情况下才需要如下方法，暂时模拟不做任何处理
 * @param in
 * @return
 */
std::vector<ArrayRef> Ycy1Io::makeBitSecret(const ArrayRef& in) const {
  // PT_BOOL && vtype == VIS_SECRET
  YACL_ENFORCE(in.eltype().isa<PtTy>(), "expected PtType, got {}", in.eltype());
  PtType in_pt_type = in.eltype().as<PtTy>()->pt_type();
  YACL_ENFORCE(in_pt_type == PT_BOOL);

  if (in_pt_type == PT_BOOL) {
    // we assume boolean is stored with byte array.
    in_pt_type = PT_U8;
  }

  const auto out_type = makeType<ycy1::BShrTy>(PT_U8, /* out_nbits */ 1);
  const size_t numel = in.numel();
  constexpr auto kCryptoType =
      yacl::crypto::SymmetricCrypto::CryptoType::AES128_CTR;
  constexpr uint128_t kAesIV = 0U;

  std::vector<ArrayRef> shares{
      {out_type, numel}, {out_type, numel}, {out_type, numel}};
  return DISPATCH_UINT_PT_TYPES(in_pt_type, "_", [&]() {
    using InT = ScalarT;
    using BShrT = uint8_t;

    auto _in = ArrayView<InT>(in);

    std::vector<BShrT> r0(numel);
    std::vector<BShrT> r1(numel);

    uint64_t counter = 0;
    yacl::crypto::FillPseudoRandom(kCryptoType, yacl::crypto::RandSeed(),
                                   kAesIV, counter, absl::MakeSpan(r0));
    yacl::crypto::FillPseudoRandom(kCryptoType, yacl::crypto::RandSeed(),
                                   kAesIV, counter, absl::MakeSpan(r1));

    auto _s0 = ArrayView<std::array<BShrT, 2>>(shares[0]);
    auto _s1 = ArrayView<std::array<BShrT, 2>>(shares[1]);
    auto _s2 = ArrayView<std::array<BShrT, 2>>(shares[2]);

    for (int64_t idx = 0; idx < in.numel(); idx++) {
      const BShrT r2 = static_cast<BShrT>(_in[idx]) - r0[idx] - r1[idx];

      _s0[idx][0] = r0[idx] & 0x1;
      _s0[idx][1] = r1[idx] & 0x1;

      _s1[idx][0] = r1[idx] & 0x1;
      _s1[idx][1] = r2 & 0x1;

      _s2[idx][0] = r2 & 0x1;
      _s2[idx][1] = r0[idx] & 0x1;
    }
    return shares;
  });
}

ArrayRef Ycy1Io::fromShares(const std::vector<ArrayRef>& shares) const {
  const auto& eltype = shares.at(0).eltype();
  // 这里拿到的shares是各个结点的数据
  // 假设原始数据[4,5] 分享后----> [[14,15],[14,15],[14,15]]  <--- shares
  if (eltype.isa<Pub2kTy>()) {
    YACL_ENFORCE(field_ == eltype.as<Ring2k>()->field());
    return shares[0].as(makeType<RingTy>(field_));
  } else if (eltype.isa<ycy1::AYcyTy>()) {
    YACL_ENFORCE(field_ == eltype.as<Ring2k>()->field());
    ArrayRef out(makeType<Pub2kTy>(field_), shares[0].numel());
    DISPATCH_ALL_FIELDS(field_, "_", [&]() {
      auto _out = ArrayView<ring2k_t>(out);
      // 遍历所有分享方数据
      for (size_t si = 0; si < shares.size(); si++) {
        // 获取每一方的数据
        auto _share = ArrayView<ring2k_t>(shares[si]);
        for (auto idx = 0; idx < shares[0].numel(); idx++) {
          if (si == 0) {
            _out[idx] = 0;
          }
          _out[idx] += _share[idx];
        }
      }
    });
    return out;
  } else if (eltype.isa<ycy1::BShrTy>()) {
    // 暂时不处理BShrTy类型，目前根据toShares方法返回值，都为AShrTy
    // BShrTy增加了数据多少bit的信息
    ArrayRef out(makeType<Pub2kTy>(field_), shares[0].numel());

    DISPATCH_ALL_FIELDS(field_, "_", [&]() {
      using OutT = ring2k_t;
      auto _out = ArrayView<OutT>(out);
      DISPATCH_UINT_PT_TYPES(
          eltype.as<ycy1::BShrTy>()->getBacktype(), "_", [&] {
            using BShrT = ScalarT;
            for (size_t si = 0; si < shares.size(); si++) {
              auto _share = ArrayView<std::array<BShrT, 2>>(shares[si]);
              for (auto idx = 0; idx < shares[0].numel(); idx++) {
                if (si == 0) {
                  _out[idx] = 0;
                }
                _out[idx] ^= _share[idx][0];
              }
            }
          });
    });
    return out;
  }
  YACL_THROW("unsupported eltype {}", eltype);
}

std::unique_ptr<Ycy1Io> makeYcy1Io(FieldType field, size_t npc) {
  YACL_ENFORCE_EQ(npc, 3u, "ycy1 is only for 3pc.");
  ycy1::registerTypes();
  return std::make_unique<Ycy1Io>(field, npc);
}

}  // namespace spu::mpc::ycy1
